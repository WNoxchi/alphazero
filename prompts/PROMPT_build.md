...
0b. Study `@notes/go-improvements-plan.md` to see the current prioritized task list.
0c. For reference, application source code is in `src/*`.
0d. Use only 1 subagent for build and test operations.

## Your Task

1. Read `@notes/go-improvements-plan.md` and select the single highest-priority 
...
6. If tests unrelated to your work fail, and they are quick to fix, fix them. 
   If they require significant work, document them in `@notes/go-improvements-plan.md` 
   as a new task item — do not derail your current task.

7. Once all validation passes:
   - Update `@notes/go-improvements-plan.md`: mark your task complete, note any 
...
8. When you discover issues, immediately update `@notes/go-improvements-plan.md` with your findings using a subagent. When resolved, update and remove the item.
...
- If you notice bugs, missing features, or improvement opportunities outside 
  your current task, add them to `@notes/go-improvements-plan.md` and move on.
...
- Keep `@AGENTS.md` operational only - status updates and progress notes belong in `@notes/go-improvements-plan.md`. A bloaded AGENTS.md pollutes every future loop's context.
